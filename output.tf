...
# Used to output the External IP in the terminal

output "instance_external_ips" {
  value = {
    vm1 = "http://${google_compute_instance.this.network_interface[0].access_config[0].nat_ip}"
    vm2 = "http://${google_compute_instance.newvminstance.network_interface[0].access_config[0].nat_ip}"
    vm3 = "http://${google_compute_instance.lizzotokyohq.network_interface[0].access_config[0].nat_ip}"
  }
